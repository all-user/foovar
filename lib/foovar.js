...
module.exports = function foovarFunc(outPath, options) {
  ...
  var _optionsResolver = optionsResolver(options),
      ...
      plainObject = _optionsResolver.plainObject;
  ...
  var body = Object.keys(this.global.scope.locals).map(function (k) {
    ...
    var foovarValueStr = 'new F(new S(' + JSON.stringify(v, replacer, 0) + ',true))';
    if (plain) {
      foovarValueStr = 'p(' + foovarValueStr + ',{options:' + plain + '})';
    }
    return _case2.default.camel(k) + ':' + foovarValueStr.replace(/^(.+)$/gm, '$1');
  }).join(comp ? ',' : ',\n');
  ...
  var codeStr = '(function(){var F=require(' + requirePathForFoovarValue + ');var S=require(' + requirePathForStylusExpression + ');' + requireConvertToPlainObject + 'module.exports={' + (comp ? '' : '\n') + body + '};})();';
