...
module.exports = function foovarFunc(outPath, options) {
  ...
  var include = _optionsResolver.include;
  var exclude = _optionsResolver.exclude;
  var noGeneratedLog = _optionsResolver.noGeneratedLog;
  var compress = _optionsResolver.compress;
  ...
  var codeStr = '(function(){var F=require(' + (TEST ? '\'' + _path2.default.resolve(process.cwd(), 'src/FoovarValue.js') + '\'' : '\'foovar/lib/FoovarValue\'') + ');var S=require(' + (TEST ? '\'' + _path2.default.resolve(process.cwd(), 'src/StylusExpression.js') + '\'' : '\'foovar/lib/StylusExpression\'') + ');module.exports={' + body + '};})();';
